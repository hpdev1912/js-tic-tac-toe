...
export function checkGameStatus(cellValues) {
  ...
  if (!Array.isArray(cellValues) || cellValues.length !== 9) {
    throw new Error("cellValues was invalid!!!");
  }
  //playing
  const result = {
  ...
  //win
  for (const checkSet of checkSetList) {
    const first = cellValues[checkSet[0]];
    const second = cellValues[checkSet[1]];
    const third = cellValues[checkSet[2]];

    if (first !== "" && first === second && second === third) {
      return {
        status:
          first === CELL_VALUE.CIRCLE ? GAME_STATUS.O_WIN : GAME_STATUS.X_WIN,
        winPositions: checkSet,
      };
    }
  }
  //end game
  const gameBoardFull = cellValues.findIndex((cellValue) => cellValue === "");

  return {
    status: gameBoardFull < 0 ? GAME_STATUS.ENDED : GAME_STATUS.PLAYING,
    winPositions: [],
  };
}
